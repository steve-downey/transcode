...
#ifndef INCLUDE_BEMAN_TRANSCODE_DETAIL_SINGLE_BYTE_ENCODE_VIEW_HPP
#define INCLUDE_BEMAN_TRANSCODE_DETAIL_SINGLE_BYTE_ENCODE_VIEW_HPP
...
template <codec C, std::ranges::random_access_range R>
    requires detail::single_byte_codec<C> && unicode_scalar_range<R>
class single_byte_encode_view : public std::ranges::view_interface<single_byte_encode_view<C, R>> {
    R base_;
    ...
    class iterator {
      ...
      public:
        ...
        constexpr char operator*() const {
            return detail::encode_single_byte_at(static_cast<char32_t>(*current_), table_);
        }

        constexpr char operator[](difference_type n) const {
            return detail::encode_single_byte_at(static_cast<char32_t>(current_[n]), table_);
        }
...
template <codec C>
struct single_byte_encode_closure {
    template <unicode_scalar_range R>
        requires std::ranges::random_access_range<R>
    constexpr auto operator()(R&& r) const {
        static_assert(detail::single_byte_codec<C>, "Codec must be single-byte");
        return single_byte_encode_view<C, std::remove_cvref_t<R>>(std::forward<R>(r));
    }

    template <unicode_scalar_range R>
        requires std::ranges::random_access_range<R>
    constexpr friend auto operator|(R&& r, const single_byte_encode_closure& self) {
        return self(std::forward<R>(r));
    }

    template <typename R>
        requires std::is_array_v<std::remove_cvref_t<R>>
    friend auto operator|(R&&, const single_byte_encode_closure&) {
        static_assert(!std::is_array_v<std::remove_cvref_t<R>>,
                      "single_byte_encode: raw arrays are not valid input; "
...
#endif // INCLUDE_BEMAN_TRANSCODE_DETAIL_SINGLE_BYTE_ENCODE_VIEW_HPP
