...
// std::codecvt / std::wstring_convert negative baseline benchmarks.
// Both APIs were deprecated in C++17.  std::wstring_convert was removed in
// C++26.  Compilation is guarded by __has_include(<codecvt>); on platforms
// where the header is absent the benchmark test case is SKIP()ed.
//
// Platform notes:
//   - libstdc++ (GCC): header present through at least C++23; may warn with
//     -Wdeprecated-declarations (suppressed here via diagnostic pragmas).
//   - libc++ (Clang): header removed as of libc++ 16; SKIP() path is taken.
