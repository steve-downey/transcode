...
// ---------------------------------------------------------------------------
// Named bulk helpers.  Each is specified as the pipeline it names, so each
// test pairs the helper against that pipeline as well as against a literal.
// ---------------------------------------------------------------------------
...
TEST_CASE("encode_to: alternate container (vector<char>)", "[bulk][named]") {
    using namespace beman::transcoding;
    const std::u32string input = U"Hi";
...
// ---------------------------------------------------------------------------
// Compile-time evaluation.  decode_to / encode_to allocate, so the container
// cannot escape constant evaluation; the lambdas reduce it to a value first.
// ---------------------------------------------------------------------------
