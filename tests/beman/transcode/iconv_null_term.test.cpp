...
TEST_CASE("null_term string literal piped to iconv_transcode_view mock identity", "[transcoding::iconv_null_term]") {
...
TEST_CASE("null_term char pointer piped to iconv_transcode_view mock identity", "[transcoding::iconv_null_term]") {
    std::array<char, 16>                     buf{};
    iconv_functions                          fns{mock_iconv_open, mock_iconv, mock_iconv_close};
    iconv_transcode_closure<iconv_functions> closure{fns, "ASCII", "ASCII", std::span(buf)};
...
TEST_CASE("null_term piped to iconv_transcode_or_error_view mock identity", "[transcoding::iconv_null_term]") {
    std::array<char, 16>                              buf{};
    iconv_functions                                   fns{mock_iconv_open, mock_iconv, mock_iconv_close};
...
TEST_CASE("iconv_transcode_to with null_term char pointer source (mock identity)", "[transcoding::iconv_null_term]") {
    iconv_functions fns{mock_iconv_open, mock_iconv, mock_iconv_close};
    const char*     s      = "world";
...
TEST_CASE("null_term piped to iconv_transcode UTF-8 identity (real iconv)", "[transcoding::iconv_null_term]") {
...
TEST_CASE("null_term piped to iconv_transcode UTF-8 to UTF-32LE (real iconv)", "[transcoding::iconv_null_term]") {
...
TEST_CASE("iconv_transcode_to with null_term source UTF-8 identity (real iconv)", "[transcoding::iconv_null_term]") {
    const char* s      = "Hello";
