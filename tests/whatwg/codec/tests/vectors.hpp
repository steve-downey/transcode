...
constexpr auto utf8_valid_1 = std::to_array<uint8_t>({
    0x7A, 0xC2, 0xA2, 0xE6, 0xB0, 0xB4, 0xF0, 0x9D, 0x84, 0x9E
}); // "z¢水𝄞"

constexpr auto utf8_invalid_overlong = std::to_array<uint8_t>({
    0xC0, 0x41
}); // Invalid ASCII 'A'

constexpr auto utf8_lonely_continuation = std::to_array<uint8_t>({
    0x80
});

constexpr auto utf8_surrogate_half = std::to_array<uint8_t>({
    0xED, 0xA0, 0x80
});

constexpr auto win1252_valid_1 = std::to_array<uint8_t>({
    0x80, 0x93, 0x94
}); // €, “, ”

constexpr auto sjis_valid_1 = std::to_array<uint8_t>({
    0x82, 0xA0
}); // あ
...
constexpr auto utf8_isolated_surrogate = std::to_array<uint8_t>({
