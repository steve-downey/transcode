...
matching P2728's structural and error-handling patterns, and not copying its
`charN_t`-typed interface. The character-typed cast adaptors P2728 provides
(`as_char8_t`, etc.) are the interop seam between the two libraries; they are
...
- **Strongest iterator category per codec** — single-byte codecs get a dedicated
  `random_access_whatwg_decode_view` instead of a uniform floor.
